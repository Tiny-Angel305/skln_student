...
class SubjectsSelectionActivity : BaseToolbarActivity(), SubjectAdapter.NextClickListener {
    ...
    private fun getSubjects() {
        ...
        else
        {
            ...
            var tempSubject = Choice()
            tempSubject.name = Student.Subject.BIOLOGY.toString()
            tempSubject.image = R.drawable.science
            tempSubject.colorString = "#000000"
            subjects.add(tempSubject)
            ...
            tempSubject = Choice()
            tempSubject.name = Student.Subject.PHYSICS.toString()
            tempSubject.image = R.drawable.physics
            tempSubject.colorString = "#9381FF"
            subjects.add(tempSubject)

            tempSubject = Choice()
            tempSubject.name = Student.Subject.CHEMISTRY.toString()
            tempSubject.image = R.drawable.chemistry
            tempSubject.colorString = "#BC69AA"
            subjects.add(tempSubject)
            ...
            tempSubject = Choice()
            tempSubject.name = Student.Subject.ACCOUNTANCY.toString()
            tempSubject.image = R.drawable.accountancy
            tempSubject.colorString = "#EF6F6C"
            subjects.add(tempSubject)

            tempSubject = Choice()
            tempSubject.name = Student.Subject.ECONOMICS.toString()
            tempSubject.image = R.drawable.economics
            tempSubject.colorString = "#EF6F6C"
            subjects.add(tempSubject)

            tempSubject = Choice()
            tempSubject.name = Student.Subject.BUSINESS_STUDIES.toString()
            tempSubject.image = R.drawable.business_studies
            tempSubject.colorString = "#EF6F6C"
            subjects.add(tempSubject)
